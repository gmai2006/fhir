...
*/
public class EncounterLocationServiceImpl implements EncounterLocationService {
private final Logger logger = Logger.getLogger(this.getClass().getName());

  private final EncounterLocationDao dao;

  @Inject
  public EncounterLocationServiceImpl(final EncounterLocationDao dao) {
      requireNonNull(dao);
      this.dao = dao;
  }

  public EncounterLocation find(String id) {
    final EncounterLocation result = dao.find(id);
    logger.info("find(EncounterLocation) - exited - return value={} result ");
    return result;
  }

  public List<EncounterLocation> select(int maxResult) {
      final List<EncounterLocation> result = dao.select(maxResult);
      logger.info("select(EncounterLocation) - exited - return value={} result ");

      return result;
  }

  public List<EncounterLocation> selectAll() {
      final List<EncounterLocation> results = dao.selectAll();
      logger.info("selectAll(EncounterLocation) - exited - return value={} result ");
      return results;
  }
  ...
  public void delete(EncounterLocation bean) {
  	requireNonNull(bean);
    logger.info("delete(EncounterLocation={}) - entered bean");

    dao.delete(bean);

    logger.info("delete(EncounterLocation) - exited - return value={} result ");
  }
}