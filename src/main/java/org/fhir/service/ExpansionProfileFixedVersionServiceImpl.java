...
*/
public class ExpansionProfileFixedVersionServiceImpl implements ExpansionProfileFixedVersionService {
private final Logger logger = Logger.getLogger(this.getClass().getName());

  private final ExpansionProfileFixedVersionDao dao;

  @Inject
  public ExpansionProfileFixedVersionServiceImpl(final ExpansionProfileFixedVersionDao dao) {
      requireNonNull(dao);
      this.dao = dao;
  }

  public ExpansionProfileFixedVersion find(String id) {
    final ExpansionProfileFixedVersion result = dao.find(id);
    logger.info("find(ExpansionProfileFixedVersion) - exited - return value={} result ");
    return result;
  }

  public List<ExpansionProfileFixedVersion> select(int maxResult) {
      final List<ExpansionProfileFixedVersion> result = dao.select(maxResult);
      logger.info("select(ExpansionProfileFixedVersion) - exited - return value={} result ");

      return result;
  }

  public List<ExpansionProfileFixedVersion> selectAll() {
      final List<ExpansionProfileFixedVersion> results = dao.selectAll();
      logger.info("selectAll(ExpansionProfileFixedVersion) - exited - return value={} result ");
      return results;
  }
  ...
  public void delete(ExpansionProfileFixedVersion bean) {
  	requireNonNull(bean);
    logger.info("delete(ExpansionProfileFixedVersion={}) - entered bean");

    dao.delete(bean);

    logger.info("delete(ExpansionProfileFixedVersion) - exited - return value={} result ");
  }
}