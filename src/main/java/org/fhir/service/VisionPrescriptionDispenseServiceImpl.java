...
*/
public class VisionPrescriptionDispenseServiceImpl implements VisionPrescriptionDispenseService {
private final Logger logger = Logger.getLogger(this.getClass().getName());

  private final VisionPrescriptionDispenseDao dao;

  @Inject
  public VisionPrescriptionDispenseServiceImpl(final VisionPrescriptionDispenseDao dao) {
      requireNonNull(dao);
      this.dao = dao;
  }

  public VisionPrescriptionDispense find(String id) {
    final VisionPrescriptionDispense result = dao.find(id);
    logger.info("find(VisionPrescriptionDispense) - exited - return value={} result ");
    return result;
  }

  public List<VisionPrescriptionDispense> select(int maxResult) {
      final List<VisionPrescriptionDispense> result = dao.select(maxResult);
      logger.info("select(VisionPrescriptionDispense) - exited - return value={} result ");

      return result;
  }

  public List<VisionPrescriptionDispense> selectAll() {
      final List<VisionPrescriptionDispense> results = dao.selectAll();
      logger.info("selectAll(VisionPrescriptionDispense) - exited - return value={} result ");
      return results;
  }
  ...
  public void delete(VisionPrescriptionDispense bean) {
  	requireNonNull(bean);
    logger.info("delete(VisionPrescriptionDispense={}) - entered bean");

    dao.delete(bean);

    logger.info("delete(VisionPrescriptionDispense) - exited - return value={} result ");
  }
}