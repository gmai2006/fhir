...
*/
public class ObservationComponentServiceImpl implements ObservationComponentService {
private final Logger logger = Logger.getLogger(this.getClass().getName());

  private final ObservationComponentDao dao;

  @Inject
  public ObservationComponentServiceImpl(final ObservationComponentDao dao) {
      requireNonNull(dao);
      this.dao = dao;
  }

  public ObservationComponent find(String id) {
    final ObservationComponent result = dao.find(id);
    logger.info("find(ObservationComponent) - exited - return value={} result ");
    return result;
  }

  public List<ObservationComponent> select(int maxResult) {
      final List<ObservationComponent> result = dao.select(maxResult);
      logger.info("select(ObservationComponent) - exited - return value={} result ");

      return result;
  }

  public List<ObservationComponent> selectAll() {
      final List<ObservationComponent> results = dao.selectAll();
      logger.info("selectAll(ObservationComponent) - exited - return value={} result ");
      return results;
  }
  ...
  public void delete(ObservationComponent bean) {
  	requireNonNull(bean);
    logger.info("delete(ObservationComponent={}) - entered bean");

    dao.delete(bean);

    logger.info("delete(ObservationComponent) - exited - return value={} result ");
  }
}