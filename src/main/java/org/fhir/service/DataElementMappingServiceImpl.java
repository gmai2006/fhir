...
*/
public class DataElementMappingServiceImpl implements DataElementMappingService {
private final Logger logger = Logger.getLogger(this.getClass().getName());

  private final DataElementMappingDao dao;

  @Inject
  public DataElementMappingServiceImpl(final DataElementMappingDao dao) {
      requireNonNull(dao);
      this.dao = dao;
  }

  public DataElementMapping find(String id) {
    final DataElementMapping result = dao.find(id);
    logger.info("find(DataElementMapping) - exited - return value={} result ");
    return result;
  }

  public List<DataElementMapping> select(int maxResult) {
      final List<DataElementMapping> result = dao.select(maxResult);
      logger.info("select(DataElementMapping) - exited - return value={} result ");

      return result;
  }

  public List<DataElementMapping> selectAll() {
      final List<DataElementMapping> results = dao.selectAll();
      logger.info("selectAll(DataElementMapping) - exited - return value={} result ");
      return results;
  }
  ...
  public void delete(DataElementMapping bean) {
  	requireNonNull(bean);
    logger.info("delete(DataElementMapping={}) - entered bean");

    dao.delete(bean);

    logger.info("delete(DataElementMapping) - exited - return value={} result ");
  }
}