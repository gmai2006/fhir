...
*/
public class DiagnosticReportImageServiceImpl implements DiagnosticReportImageService {
private final Logger logger = Logger.getLogger(this.getClass().getName());

  private final DiagnosticReportImageDao dao;

  @Inject
  public DiagnosticReportImageServiceImpl(final DiagnosticReportImageDao dao) {
      requireNonNull(dao);
      this.dao = dao;
  }

  public DiagnosticReportImage find(String id) {
    final DiagnosticReportImage result = dao.find(id);
    logger.info("find(DiagnosticReportImage) - exited - return value={} result ");
    return result;
  }

  public List<DiagnosticReportImage> select(int maxResult) {
      final List<DiagnosticReportImage> result = dao.select(maxResult);
      logger.info("select(DiagnosticReportImage) - exited - return value={} result ");

      return result;
  }

  public List<DiagnosticReportImage> selectAll() {
      final List<DiagnosticReportImage> results = dao.selectAll();
      logger.info("selectAll(DiagnosticReportImage) - exited - return value={} result ");
      return results;
  }
  ...
  public void delete(DiagnosticReportImage bean) {
  	requireNonNull(bean);
    logger.info("delete(DiagnosticReportImage={}) - entered bean");

    dao.delete(bean);

    logger.info("delete(DiagnosticReportImage) - exited - return value={} result ");
  }
}