...
*/
public class EligibilityResponseErrorServiceImpl implements EligibilityResponseErrorService {
private final Logger logger = Logger.getLogger(this.getClass().getName());

  private final EligibilityResponseErrorDao dao;

  @Inject
  public EligibilityResponseErrorServiceImpl(final EligibilityResponseErrorDao dao) {
      requireNonNull(dao);
      this.dao = dao;
  }

  public EligibilityResponseError find(String id) {
    final EligibilityResponseError result = dao.find(id);
    logger.info("find(EligibilityResponseError) - exited - return value={} result ");
    return result;
  }

  public List<EligibilityResponseError> select(int maxResult) {
      final List<EligibilityResponseError> result = dao.select(maxResult);
      logger.info("select(EligibilityResponseError) - exited - return value={} result ");

      return result;
  }

  public List<EligibilityResponseError> selectAll() {
      final List<EligibilityResponseError> results = dao.selectAll();
      logger.info("selectAll(EligibilityResponseError) - exited - return value={} result ");
      return results;
  }
  ...
  public void delete(EligibilityResponseError bean) {
  	requireNonNull(bean);
    logger.info("delete(EligibilityResponseError={}) - entered bean");

    dao.delete(bean);

    logger.info("delete(EligibilityResponseError) - exited - return value={} result ");
  }
}