...
*/
public class ProcessRequestItemServiceImpl implements ProcessRequestItemService {
private final Logger logger = Logger.getLogger(this.getClass().getName());

  private final ProcessRequestItemDao dao;

  @Inject
  public ProcessRequestItemServiceImpl(final ProcessRequestItemDao dao) {
      requireNonNull(dao);
      this.dao = dao;
  }

  public ProcessRequestItem find(String id) {
    final ProcessRequestItem result = dao.find(id);
    logger.info("find(ProcessRequestItem) - exited - return value={} result ");
    return result;
  }

  public List<ProcessRequestItem> select(int maxResult) {
      final List<ProcessRequestItem> result = dao.select(maxResult);
      logger.info("select(ProcessRequestItem) - exited - return value={} result ");

      return result;
  }

  public List<ProcessRequestItem> selectAll() {
      final List<ProcessRequestItem> results = dao.selectAll();
      logger.info("selectAll(ProcessRequestItem) - exited - return value={} result ");
      return results;
  }
  ...
  public void delete(ProcessRequestItem bean) {
  	requireNonNull(bean);
    logger.info("delete(ProcessRequestItem={}) - entered bean");

    dao.delete(bean);

    logger.info("delete(ProcessRequestItem) - exited - return value={} result ");
  }
}