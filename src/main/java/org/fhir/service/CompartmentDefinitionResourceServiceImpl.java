...
*/
public class CompartmentDefinitionResourceServiceImpl implements CompartmentDefinitionResourceService {
private final Logger logger = Logger.getLogger(this.getClass().getName());

  private final CompartmentDefinitionResourceDao dao;

  @Inject
  public CompartmentDefinitionResourceServiceImpl(final CompartmentDefinitionResourceDao dao) {
      requireNonNull(dao);
      this.dao = dao;
  }

  public CompartmentDefinitionResource find(String id) {
    final CompartmentDefinitionResource result = dao.find(id);
    logger.info("find(CompartmentDefinitionResource) - exited - return value={} result ");
    return result;
  }

  public List<CompartmentDefinitionResource> select(int maxResult) {
      final List<CompartmentDefinitionResource> result = dao.select(maxResult);
      logger.info("select(CompartmentDefinitionResource) - exited - return value={} result ");

      return result;
  }

  public List<CompartmentDefinitionResource> selectAll() {
      final List<CompartmentDefinitionResource> results = dao.selectAll();
      logger.info("selectAll(CompartmentDefinitionResource) - exited - return value={} result ");
      return results;
  }
  ...
  public void delete(CompartmentDefinitionResource bean) {
  	requireNonNull(bean);
    logger.info("delete(CompartmentDefinitionResource={}) - entered bean");

    dao.delete(bean);

    logger.info("delete(CompartmentDefinitionResource) - exited - return value={} result ");
  }
}