...
*/
public class DataRequirementDateFilterServiceImpl implements DataRequirementDateFilterService {
private final Logger logger = Logger.getLogger(this.getClass().getName());

  private final DataRequirementDateFilterDao dao;

  @Inject
  public DataRequirementDateFilterServiceImpl(final DataRequirementDateFilterDao dao) {
      requireNonNull(dao);
      this.dao = dao;
  }

  public DataRequirementDateFilter find(String id) {
    final DataRequirementDateFilter result = dao.find(id);
    logger.info("find(DataRequirementDateFilter) - exited - return value={} result ");
    return result;
  }

  public List<DataRequirementDateFilter> select(int maxResult) {
      final List<DataRequirementDateFilter> result = dao.select(maxResult);
      logger.info("select(DataRequirementDateFilter) - exited - return value={} result ");

      return result;
  }

  public List<DataRequirementDateFilter> selectAll() {
      final List<DataRequirementDateFilter> results = dao.selectAll();
      logger.info("selectAll(DataRequirementDateFilter) - exited - return value={} result ");
      return results;
  }
  ...
  public void delete(DataRequirementDateFilter bean) {
  	requireNonNull(bean);
    logger.info("delete(DataRequirementDateFilter={}) - entered bean");

    dao.delete(bean);

    logger.info("delete(DataRequirementDateFilter) - exited - return value={} result ");
  }
}