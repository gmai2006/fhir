...
public class ScheduleResource {
   ...
   */
	@PUT
	@Consumes(MediaType.APPLICATION_JSON)
	public Schedule createOrUpdate(Schedule obj) {
   ...
   * @param obj - instance of Schedule
   * @return Schedule
   */
	@Consumes(MediaType.APPLICATION_JSON)
	@POST
   ...
   * @param id - instance of Schedule
   * @return Schedule
   */
  @GET
  @Path("{id}")
  public Response find(@PathParam("id") String id) {
  	Schedule result = this.service.find(id);
  	if (null == result) {
  ...
  /**
   * Select all Schedule with limit of returned records
  ...
  public Response findWithLimit(@PathParam("max") String max) {
  	...
  	List<Schedule> result = service.select(input);
  ...
  /**
   * Query Schedule based on basic field names
  ...
  public Response findByField(@Context UriInfo info) {
  	...
  	else if (FhirConstant.SCHEDULE_FIELDS.contains(first)) {
  		java.util.Map<String, String> params = QueryParser.parse(str);
  ...
  /**
   * Query Schedule by composite fields
  ...
  */
  ...
  public Response type(@Context UriInfo info) {
  	...
  	java.util.Map<String, String> params = QueryParser.parse(str);
  	List<Schedule> result = this.service.findByServiceType(new QueryBuilder(params));
  ...
  */
  @GET
  @Path("actor")
  public Response actor(@Context UriInfo info) {
  	MultivaluedMap<String, String> parameters = info.getQueryParameters();
  	if (null == parameters || parameters.isEmpty()) {
  		return Response.status(Response.Status.OK).entity(service.select(50)).build();
  	}
  	...
  	java.util.Map<String, String> params = QueryParser.parse(str);
  	List<Schedule> result = this.service.findByActor(new QueryBuilder(params));
