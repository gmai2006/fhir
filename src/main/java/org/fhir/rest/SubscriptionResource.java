...
public class SubscriptionResource {
   ...
   */
	@PUT
	@Consumes(MediaType.APPLICATION_JSON)
	public Subscription createOrUpdate(Subscription obj) {
   ...
   * @param obj - instance of Subscription
   * @return Subscription
   */
	@Consumes(MediaType.APPLICATION_JSON)
	@POST
   ...
   * @param id - instance of Subscription
   * @return Subscription
   */
  @GET
  @Path("{id}")
  public Response find(@PathParam("id") String id) {
  	Subscription result = this.service.find(id);
  	if (null == result) {
  ...
  /**
   * Select all Subscription with limit of returned records
   * @param max - number of records
   * @return a list Subscription
   */
  @GET
  @Path("select/{max}")
  public Response findWithLimit(@PathParam("max") String max) {
  	...
  	List<Subscription> result = service.select(input);
  ...
  /**
   * Query Subscription based on basic field names
  ...
  public Response findByField(@Context UriInfo info) {
  	...
  	else if (FhirConstant.SUBSCRIPTION_FIELDS.contains(first)) {
  		java.util.Map<String, String> params = QueryParser.parse(str);
  ...
  /**
   * Query Subscription by composite fields
   * Descr: A tag to be added to the resource matching the criteria
   * Type: token
  */
  @GET
  @Path("addtag")
  public Response addtag(@Context UriInfo info) {
  	MultivaluedMap<String, String> parameters = info.getQueryParameters();
  	if (null == parameters || parameters.isEmpty()) {
  		return Response.status(Response.Status.OK).entity(service.select(50)).build();
  	}
  	...
  	java.util.Map<String, String> params = QueryParser.parse(str);
  	List<Subscription> result = this.service.findByTag(new QueryBuilder(params));
