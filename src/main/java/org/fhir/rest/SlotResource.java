...
public class SlotResource {
   ...
   */
	@PUT
	@Consumes(MediaType.APPLICATION_JSON)
	public Slot createOrUpdate(Slot obj) {
   ...
   */
	...
	/**
   * Get Slot by its ID
   * @param id - instance of Slot
   * @return Slot
   */
  @GET
  @Path("{id}")
  public Response find(@PathParam("id") String id) {
  	Slot result = this.service.find(id);
  	if (null == result) {
  ...
  /**
   * Select all Slot with limit of returned records
  ...
  public Response findWithLimit(@PathParam("max") String max) {
  	...
  	List<Slot> result = service.select(input);
  ...
  /**
   * Query Slot based on basic field names
  ...
  public Response findByField(@Context UriInfo info) {
  	...
  	else if (FhirConstant.SLOT_FIELDS.contains(first)) {
  		java.util.Map<String, String> params = QueryParser.parse(str);
  ...
  /**
   * Query Slot by composite fields
   * Descr: The type of appointments that can be booked into the slot
   * Type: token
  */
  @GET
  @Path("slottype")
  public Response slottype(@Context UriInfo info) {
  	MultivaluedMap<String, String> parameters = info.getQueryParameters();
  	if (null == parameters || parameters.isEmpty()) {
  		return Response.status(Response.Status.OK).entity(service.select(50)).build();
  	}
  	...
  	java.util.Map<String, String> params = QueryParser.parse(str);
  	List<Slot> result = this.service.findByServiceType(new QueryBuilder(params));
  ...
  */
  @GET
  @Path("schedule")
  public Response schedule(@Context UriInfo info) {
  	MultivaluedMap<String, String> parameters = info.getQueryParameters();
  	if (null == parameters || parameters.isEmpty()) {
  		return Response.status(Response.Status.OK).entity(service.select(50)).build();
  	}
  	...
  	java.util.Map<String, String> params = QueryParser.parse(str);
  	List<Slot> result = this.service.findBySchedule(new QueryBuilder(params));
