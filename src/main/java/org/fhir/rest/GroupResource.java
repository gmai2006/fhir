...
public class GroupResource {
   ...
   */
	@PUT
	@Consumes(MediaType.APPLICATION_JSON)
	public Group createOrUpdate(Group obj) {
   ...
   */
	...
	/**
   * Get Group by its ID
   * @param id - instance of Group
   * @return Group
   */
  @GET
  @Path("{id}")
  public Response find(@PathParam("id") String id) {
  	Group result = this.service.find(id);
  	if (null == result) {
  ...
  /**
   * Select all Group with limit of returned records
  ...
  public Response findWithLimit(@PathParam("max") String max) {
  	...
  	List<Group> result = service.select(input);
  ...
  /**
   * Query Group based on basic field names
  ...
  public Response findByField(@Context UriInfo info) {
  	...
  	else if (FhirConstant.GROUP_FIELDS.contains(first)) {
  		java.util.Map<String, String> params = QueryParser.parse(str);
  ...
  /**
   * Query Group by composite fields
  ...
  */
  ...
  public Response code(@Context UriInfo info) {
  	...
  	java.util.Map<String, String> params = QueryParser.parse(str);
  	List<Group> result = this.service.findByCode(new QueryBuilder(params));
