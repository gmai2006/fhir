...
public class TestReportResource {
   ...
   */
	@PUT
	@Consumes(MediaType.APPLICATION_JSON)
	public TestReport createOrUpdate(TestReport obj) {
   ...
   * @param obj - instance of TestReport
   * @return TestReport
   */
	@Consumes(MediaType.APPLICATION_JSON)
	@POST
   ...
   */
	...
	/**
   * Get TestReport by its ID
   * @param id - instance of TestReport
   * @return TestReport
   */
  @GET
  @Path("{id}")
  public Response find(@PathParam("id") String id) {
  	TestReport result = this.service.find(id);
  	if (null == result) {
  ...
  /**
   * Select all TestReport with limit of returned records
   * @param max - number of records
   * @return a list TestReport
   */
  @GET
  @Path("select/{max}")
  public Response findWithLimit(@PathParam("max") String max) {
  	...
  	List<TestReport> result = service.select(input);
  ...
  /**
   * Query TestReport based on basic field names
  ...
  public Response findByField(@Context UriInfo info) {
  	...
  	else if (FhirConstant.TESTREPORT_FIELDS.contains(first)) {
  		java.util.Map<String, String> params = QueryParser.parse(str);
  ...
  /**
   * Query TestReport by composite fields
   * Descr: The test script executed to produce this report
   * Type: reference
  */
  @GET
  @Path("testscript")
  public Response testscript(@Context UriInfo info) {
  	MultivaluedMap<String, String> parameters = info.getQueryParameters();
  	if (null == parameters || parameters.isEmpty()) {
  		return Response.status(Response.Status.OK).entity(service.select(50)).build();
  	}
  	...
  	java.util.Map<String, String> params = QueryParser.parse(str);
  	List<TestReport> result = this.service.findByTestScript(new QueryBuilder(params));
