...
public class EndpointResource {
  ...
  public Response find(@PathParam("id") String id) {
  	Endpoint result = this.service.find(id);
  	if (null == result) {
  ...
  public Response findWithLimit(@PathParam("max") String max) {
  	...
  	List<Endpoint> result = service.select(input);
  ...
  public Response findByField(@Context UriInfo info) {
  	...
  	else if (FhirConstant.ENDPOINT_FIELDS.contains(first)) {
  		java.util.Map<String, String> params = QueryParser.parse(str);
  ...
  */
  ...
  public Response organization(@Context UriInfo info) {
  	...
  	java.util.Map<String, String> params = QueryParser.parse(str);
  	List<Endpoint> result = this.service.findByManagingOrganization(new QueryBuilder(params));
  ...
  */
  @GET
  @Path("payloadtype")
  public Response payloadtype(@Context UriInfo info) {
  	MultivaluedMap<String, String> parameters = info.getQueryParameters();
  	if (null == parameters || parameters.isEmpty()) {
  		return Response.status(Response.Status.OK).entity(service.select(50)).build();
  	}
  	...
  	java.util.Map<String, String> params = QueryParser.parse(str);
  	List<Endpoint> result = this.service.findByPayloadType(new QueryBuilder(params));
