...
public class GoalResource {
   ...
   */
	@PUT
	@Consumes(MediaType.APPLICATION_JSON)
	public Goal createOrUpdate(Goal obj) {
   ...
   */
	...
	/**
   * Get Goal by its ID
   * @param id - instance of Goal
   * @return Goal
   */
  @GET
  @Path("{id}")
  public Response find(@PathParam("id") String id) {
  	Goal result = this.service.find(id);
  	if (null == result) {
  ...
  /**
   * Select all Goal with limit of returned records
  ...
  public Response findWithLimit(@PathParam("max") String max) {
  	...
  	List<Goal> result = service.select(input);
  ...
  /**
   * Query Goal based on basic field names
  ...
  public Response findByField(@Context UriInfo info) {
  	...
  	else if (FhirConstant.GOAL_FIELDS.contains(first)) {
  		java.util.Map<String, String> params = QueryParser.parse(str);
  ...
  /**
   * Query Goal by composite fields
  ...
  */
  ...
  public Response subject(@Context UriInfo info) {
  	...
  	java.util.Map<String, String> params = QueryParser.parse(str);
  	List<Goal> result = this.service.findBySubject(new QueryBuilder(params));
