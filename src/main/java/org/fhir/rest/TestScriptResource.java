...
public class TestScriptResource {
   ...
   */
	@PUT
	@Consumes(MediaType.APPLICATION_JSON)
	public TestScript createOrUpdate(TestScript obj) {
	...
	/**
	 * InIdempotent method
   * Update existing TestScript
   * @param obj - instance of TestScript
   * @return TestScript
   */
	@Consumes(MediaType.APPLICATION_JSON)
	@POST
   ...
   */
	...
	/**
   * Get TestScript by its ID
   * @param id - instance of TestScript
   * @return TestScript
   */
  @GET
  @Path("{id}")
  public Response find(@PathParam("id") String id) {
  	TestScript result = this.service.find(id);
  	if (null == result) {
  ...
  /**
   * Select all TestScript with limit of returned records
   * @param max - number of records
   * @return a list TestScript
   */
  @GET
  @Path("select/{max}")
  public Response findWithLimit(@PathParam("max") String max) {
  	...
  	List<TestScript> result = service.select(input);
  ...
  /**
   * Query TestScript based on basic field names
   * @param UriInfo - UriInfo
   * @return list of TestScript
   */
  @GET
  public Response findByField(@Context UriInfo info) {
  	...
  	else if (FhirConstant.TESTSCRIPT_FIELDS.contains(first)) {
  		java.util.Map<String, String> params = QueryParser.parse(str);
  ...
  /**
   * Query TestScript by composite fields
  ...
  */
  ...
  public Response jurisdiction(@Context UriInfo info) {
  	...
  	java.util.Map<String, String> params = QueryParser.parse(str);
  	List<TestScript> result = this.service.findByJurisdiction(new QueryBuilder(params));
