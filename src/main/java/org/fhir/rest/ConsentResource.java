...
public class ConsentResource {
   ...
   */
	@PUT
	@Consumes(MediaType.APPLICATION_JSON)
	public Consent createOrUpdate(Consent obj) {
   ...
   */
	...
	/**
   * Get Consent by its ID
   * @param id - instance of Consent
   * @return Consent
   */
  @GET
  @Path("{id}")
  public Response find(@PathParam("id") String id) {
  	Consent result = this.service.find(id);
  	if (null == result) {
  ...
  /**
   * Select all Consent with limit of returned records
   * @param max - number of records
   * @return a list Consent
   */
  @GET
  @Path("select/{max}")
  public Response findWithLimit(@PathParam("max") String max) {
  	...
  	List<Consent> result = service.select(input);
  ...
  /**
   * Query Consent based on basic field names
  ...
  public Response findByField(@Context UriInfo info) {
  	...
  	else if (FhirConstant.CONSENT_FIELDS.contains(first)) {
  		java.util.Map<String, String> params = QueryParser.parse(str);
  ...
  /**
   * Query Consent by composite fields
  ...
  */
  ...
  public Response category(@Context UriInfo info) {
  	...
  	java.util.Map<String, String> params = QueryParser.parse(str);
  	List<Consent> result = this.service.findByCategory(new QueryBuilder(params));
  ...
  */
  @GET
  @Path("consentor")
  public Response consentor(@Context UriInfo info) {
  	MultivaluedMap<String, String> parameters = info.getQueryParameters();
  	if (null == parameters || parameters.isEmpty()) {
  		return Response.status(Response.Status.OK).entity(service.select(50)).build();
  	}
  	...
  	java.util.Map<String, String> params = QueryParser.parse(str);
  	List<Consent> result = this.service.findByConsentingParty(new QueryBuilder(params));
  ...
  */
  ...
  public Response organization(@Context UriInfo info) {
  	...
  	java.util.Map<String, String> params = QueryParser.parse(str);
  	List<Consent> result = this.service.findByOrganization(new QueryBuilder(params));
