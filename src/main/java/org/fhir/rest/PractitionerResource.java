...
public class PractitionerResource {
   ...
   */
	@PUT
	@Consumes(MediaType.APPLICATION_JSON)
	public Practitioner createOrUpdate(Practitioner obj) {
   ...
   * @param obj - instance of Practitioner
   * @return Practitioner
   */
	@Consumes(MediaType.APPLICATION_JSON)
	@POST
   ...
   */
	...
	/**
   * Get Practitioner by its ID
   * @param id - instance of Practitioner
   * @return Practitioner
   */
  @GET
  @Path("{id}")
  public Response find(@PathParam("id") String id) {
  	Practitioner result = this.service.find(id);
  	if (null == result) {
  ...
  /**
   * Select all Practitioner with limit of returned records
   * @param max - number of records
   * @return a list Practitioner
   */
  @GET
  @Path("select/{max}")
  public Response findWithLimit(@PathParam("max") String max) {
  	...
  	List<Practitioner> result = service.select(input);
  ...
  /**
   * Query Practitioner based on basic field names
  ...
  public Response findByField(@Context UriInfo info) {
  	...
  	else if (FhirConstant.PRACTITIONER_FIELDS.contains(first)) {
  		java.util.Map<String, String> params = QueryParser.parse(str);
  ...
  /**
   * Query Practitioner by composite fields
   * Descr: One of the languages that the practitioner can communicate with
   * Type: token
  */
  @GET
  @Path("communication")
  public Response communication(@Context UriInfo info) {
  	MultivaluedMap<String, String> parameters = info.getQueryParameters();
  	if (null == parameters || parameters.isEmpty()) {
  		return Response.status(Response.Status.OK).entity(service.select(50)).build();
  	}
  	...
  	java.util.Map<String, String> params = QueryParser.parse(str);
  	List<Practitioner> result = this.service.findByCommunication(new QueryBuilder(params));
