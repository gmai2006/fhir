/*
 * #%L
 * FHIR Implementation
 * %%
 * Copyright (C) 2018 DataScience 9 LLC
 * %%
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 * 
 *      http://www.apache.org/licenses/LICENSE-2.0
 * 
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 * #L%
 */
 
/**
 * This code is 100% AUTO generated. Please do not modify it DIRECTLY
 * If you need new features or function or changes please update the templates
 * then submit the template through our web interface.  
 */

package org.fhir.rest;

import static java.util.Objects.requireNonNull;

import java.util.List;

import java.util.logging.Logger;

import javax.ws.rs.Consumes;
import javax.ws.rs.GET;
import javax.ws.rs.POST;
import javax.ws.rs.DELETE;
import javax.ws.rs.Path;
import javax.ws.rs.PathParam;
import javax.ws.rs.Produces;
import javax.ws.rs.WebApplicationException;
import javax.ws.rs.core.MediaType;
import javax.ws.rs.core.Response;
import javax.ws.rs.QueryParam;
import javax.ws.rs.PUT;

import com.google.inject.Inject;

import org.fhir.pojo.DiagnosticReport;
import org.fhir.service.DiagnosticReportService;

@Path("/DiagnosticReport")
@Consumes(MediaType.APPLICATION_JSON)
@Produces({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
/**
 * Auto generated RESTful class
 */
public class DiagnosticReportResource {
  private final Logger logger = Logger.getLogger(this.getClass().getName());
  private final DiagnosticReportService service;

  @Inject
  public DiagnosticReportResource(final DiagnosticReportService service) {
  	requireNonNull(service);
    this.service = service;
  }

  @PUT
  @Consumes(MediaType.APPLICATION_JSON)
  public DiagnosticReport create(DiagnosticReport obj) {
  	return this.service.create(obj);
  }

  @Consumes(MediaType.APPLICATION_JSON)
  @POST
  public DiagnosticReport update( DiagnosticReport obj) {
  	return this.service.update(obj);
  }

  @GET
  @Consumes(MediaType.APPLICATION_JSON)
  @Path("{id}")
  public DiagnosticReport find(@PathParam("id") String id) {
  	return this.service.find(id);
  }

  @GET
  @Consumes(MediaType.APPLICATION_JSON)
  @Path("selectAll")
  public List<DiagnosticReport> findAll() {
  	return this.service.selectAll();
  }

  @GET
  @Path("/select/{max}")
  public List<DiagnosticReport> findWithLimit(@PathParam("max") String max) {
  	Integer input = null;
  	try {
  		input = Integer.valueOf(max);
  	} catch (Exception ex) {
  		throw new WebApplicationException(Response.Status.BAD_REQUEST);
  	}
      return service.select(input);
  }

  /** TBD*/
  /**
  * Descr: Reference to the procedure request.
  * expr: basedOn
  * Xpath: f:DiagnosticReport/f:basedOn
  * Type: reference
  * Code: basedon
  */
  @GET
  @Path("basedOn")
  @Produces({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON}) 
  public DiagnosticReport basedon() {
  	return new DiagnosticReport();
  }
  /**
  * Descr: Which diagnostic discipline/department created the report
  * expr: category
  * Xpath: f:DiagnosticReport/f:category
  * Type: token
  * Code: category
  */
  @GET
  @Path("category")
  @Produces({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON}) 
  public DiagnosticReport category(@QueryParam("category")String category) {
  	return new DiagnosticReport();
  }
  /**
  * Descr: Healthcare event (Episode of Care or Encounter) related to the report
  * expr: context
  * Xpath: f:DiagnosticReport/f:context
  * Type: reference
  * Code: context
  */
  @GET
  @Path("context")
  @Produces({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON}) 
  public DiagnosticReport context() {
  	return new DiagnosticReport();
  }
  /**
  * Descr: A coded diagnosis on the report
  * expr: codedDiagnosis
  * Xpath: f:DiagnosticReport/f:codedDiagnosis
  * Type: token
  * Code: diagnosis
  */
  @GET
  @Path("codedDiagnosis")
  @Produces({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON}) 
  public DiagnosticReport diagnosis(@QueryParam("diagnosis")String diagnosis) {
  	return new DiagnosticReport();
  }
  /**
  * Descr: A reference to the image source.
  * expr: link
  * Xpath: f:DiagnosticReport/f:image/f:link
  * Type: reference
  * Code: image
  */
  @GET
  @Path("link")
  @Produces({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON}) 
  public DiagnosticReport image() {
  	return new DiagnosticReport();
  }
  /**
  * Descr: When the report was issued
  * expr: issued
  * Xpath: f:DiagnosticReport/f:issued
  * Type: date
  * Code: issued
  */
  @GET
  @Path("issued")
  @Produces({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON}) 
  public DiagnosticReport issued(@QueryParam("issued")String issued) {
  	return new DiagnosticReport();
  }
  /**
  * Descr: Who was the source of the report (organization)
  * expr: actor
  * Xpath: f:DiagnosticReport/f:performer/f:actor
  * Type: reference
  * Code: performer
  */
  @GET
  @Path("actor")
  @Produces({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON}) 
  public DiagnosticReport performer() {
  	return new DiagnosticReport();
  }
  /**
  * Descr: Link to an atomic result (observation resource)
  * expr: result
  * Xpath: f:DiagnosticReport/f:result
  * Type: reference
  * Code: result
  */
  @GET
  @Path("result")
  @Produces({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON}) 
  public DiagnosticReport result() {
  	return new DiagnosticReport();
  }
  /**
  * Descr: The specimen details
  * expr: specimen
  * Xpath: f:DiagnosticReport/f:specimen
  * Type: reference
  * Code: specimen
  */
  @GET
  @Path("specimen")
  @Produces({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON}) 
  public DiagnosticReport specimen() {
  	return new DiagnosticReport();
  }
  /**
  * Descr: The status of the report
  * expr: status
  * Xpath: f:DiagnosticReport/f:status
  * Type: token
  * Code: status
  */
  @GET
  @Path("status")
  @Produces({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON}) 
  public DiagnosticReport status(@QueryParam("status")String status) {
  	return new DiagnosticReport();
  }
  /**
  * Descr: The subject of the report
  * expr: subject
  * Xpath: f:DiagnosticReport/f:subject
  * Type: reference
  * Code: subject
  */
  @GET
  @Path("subject")
  @Produces({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON}) 
  public DiagnosticReport subject() {
  	return new DiagnosticReport();
  }
}