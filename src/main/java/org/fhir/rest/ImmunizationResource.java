...
public class ImmunizationResource {
   ...
   */
	@PUT
	@Consumes(MediaType.APPLICATION_JSON)
	public Immunization createOrUpdate(Immunization obj) {
   ...
   * @param obj - instance of Immunization
   * @return Immunization
   */
	@Consumes(MediaType.APPLICATION_JSON)
	@POST
   ...
   */
	...
	/**
   * Get Immunization by its ID
   * @param id - instance of Immunization
   * @return Immunization
   */
  @GET
  @Path("{id}")
  public Response find(@PathParam("id") String id) {
  	Immunization result = this.service.find(id);
  	if (null == result) {
  ...
  /**
   * Select all Immunization with limit of returned records
   * @param max - number of records
   * @return a list Immunization
   */
  @GET
  @Path("select/{max}")
  public Response findWithLimit(@PathParam("max") String max) {
  	...
  	List<Immunization> result = service.select(input);
  ...
  /**
   * Query Immunization based on basic field names
  ...
  /**
   * Query Immunization by composite fields
   * Descr: Vaccine Product Administered
   * Type: token
  */
  @GET
  @Path("vaccinecode")
  public Response vaccinecode(@Context UriInfo info) {
  	MultivaluedMap<String, String> parameters = info.getQueryParameters();
  	if (null == parameters || parameters.isEmpty()) {
  		return Response.status(Response.Status.OK).entity(service.select(50)).build();
  	}
  	...
  	java.util.Map<String, String> params = QueryParser.parse(str);
  	List<Immunization> result = this.service.findByVaccineCode(new QueryBuilder(params));
  ...
  */
  ...
  public Response location(@Context UriInfo info) {
  	...
  	java.util.Map<String, String> params = QueryParser.parse(str);
  	List<Immunization> result = this.service.findByLocation(new QueryBuilder(params));
  ...
  */
  @GET
  @Path("manufacturer")
  public Response manufacturer(@Context UriInfo info) {
  	MultivaluedMap<String, String> parameters = info.getQueryParameters();
  	if (null == parameters || parameters.isEmpty()) {
  		return Response.status(Response.Status.OK).entity(service.select(50)).build();
  	}
  	...
  	java.util.Map<String, String> params = QueryParser.parse(str);
  	List<Immunization> result = this.service.findByManufacturer(new QueryBuilder(params));
