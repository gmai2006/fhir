...
public class BundleResource {
   ...
   */
	@PUT
	@Consumes(MediaType.APPLICATION_JSON)
	public Bundle createOrUpdate(Bundle obj) {
   ...
   */
	...
	/**
   * Get Bundle by its ID
   * @param id - instance of Bundle
   * @return Bundle
   */
  @GET
  @Path("{id}")
  public Response find(@PathParam("id") String id) {
  	Bundle result = this.service.find(id);
  	if (null == result) {
  ...
  /**
   * Select all Bundle with limit of returned records
   * @param max - number of records
   * @return a list Bundle
   */
  @GET
  @Path("select/{max}")
  public Response findWithLimit(@PathParam("max") String max) {
  	...
  	List<Bundle> result = service.select(input);
  ...
  /**
   * Query Bundle based on basic field names
  ...
  public Response findByField(@Context UriInfo info) {
  	...
  	else if (FhirConstant.BUNDLE_FIELDS.contains(first)) {
  		java.util.Map<String, String> params = QueryParser.parse(str);
